...
if(empty($error))
{
$update=mysqli_query($dbc,"UPDATE cms SET fullname='$fullname', phone='$phone', email='$email', address='$address',company='$company', jobtitle='$jobtitle',website='$website',
	last_modified_by='$last_modified_by' WHERE (id=$id AND cms_id={$row['cms_id']} AND $tenant) LIMIT 1");
...
<h5>Company</h5>
<input type="text" placeholder="Add a Company" id="company" name="company"  size="50" maxlength="100" 
value="<?php if(isset($row['company'])) {echo $row['company'];} ?>" />
<h5>Job Title</h5>
<input type="text" placeholder="Add a Job Title" id="jobtitle" name="jobtitle"  size="50" maxlength="100" value="<?php if(isset($row['jobtitle'])) {echo $row['jobtitle'];} ?>" />
<h5>Website</h5>
<label> Please website should be in this format (http://yahoo.com) only. No www. </label>
<input type="text" placeholder="Add a Website" id="website" name="website"  size="50" maxlength="100" 
value="<?php if(isset($row['website'])){ echo $row['website'];} ?>" />
</div>
